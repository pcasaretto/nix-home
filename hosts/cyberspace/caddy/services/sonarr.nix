...
EOF
  ...
  systemd.services.sonarr-telegram-setup = {
    description = "Configure Sonarr Telegram notifications";
    after = [ "sonarr.service" ];
    wantedBy = [ "multi-user.target" ];
    serviceConfig = {
      Type = "oneshot";
    ...
    script = ''
      ...
      if ${pkgs.curl}/bin/curl -sf http://127.0.0.1:${toString ports.media.sonarr}/api/v3/notification -H "X-Api-Key: $API_KEY" | ${pkgs.jq}/bin/jq -e '.[] | select(.name == "Telegram")' > /dev/null 2>&1; then
        echo "Telegram notification already configured"
        exit 0
      fi

      echo "Configuring Telegram notification for Sonarr..."
